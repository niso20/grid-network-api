...
class Starlette:
    ...
    def __init__(
        ...
        Parameters:
            ...
            middleware: A list of middlewares to run for every request. A starlette
                application will always automatically include two middlewares classes.
                `ServerErrorMiddleware` is added as the very outermost middlewares, to handle
                any uncaught errors occurring anywhere in the entire stack.
                `ExceptionMiddleware` is added as the very innermost middlewares, to deal
    ...
    def add_middleware(
        ...
        **kwargs: P.kwargs,
        ...
        if self.middleware_stack is not None:  # pragma: no cover
            raise RuntimeError("Cannot add middlewares after an application has started")
    ...
    def middleware(self, middleware_type: str) -> typing.Callable:  # type: ignore[type-arg]
        ...
        >>> app = Starlette(middlewares=middlewares)
        """
        warnings.warn(
            "The `middlewares` decorator is deprecated, and will be removed in version 1.0.0. "
            "Refer to https://www.starlette.io/middleware/#using-middleware for recommended approach.",
            DeprecationWarning,
        )
        assert middleware_type == "http", 'Currently only middlewares("http") is supported.'
